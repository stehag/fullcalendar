...
export async function writeMonorepoArchives(monorepoStruct) {
    await Promise.all(getArchiveRootDirs(monorepoStruct).map((rootDir) => createArchive(rootDir)));
}
export async function deleteMonorepoArchives(monorepoStruct) {
    await Promise.all(getArchiveRootDirs(monorepoStruct).map((rootDir) => deleteArchives(rootDir)));
}
async function createArchive(rootDir) {
    ...
    ['README.md', 'LICENSE.md'].forEach((subpath) => {
        archive.file(joinPaths(rootDir, subpath), { name: `${archiveId}/${subpath}` });
    });
    archive.directory(joinPaths(bundleDir, 'examples'), `${archiveId}/examples`);
    archive.glob(`dist/*${iifeSubextension}.+(js|min.js)`, { cwd: bundleDir }, { prefix: archiveId });
    const subpaths = await globby(`packages/*/dist/**/*${iifeSubextension}.+(js|min.js)`, { cwd: rootDir });
    for (const subpath of subpaths) {
        const subpathParts = subpath.split(pathSeparator);
        subpathParts.splice(2, 1); // remove 'dist'
        archive.file(joinPaths(rootDir, subpath), { name: [archiveId].concat(subpathParts).join('/') });
    }
    return archive.finalize();
}
async function deleteArchives(rootDir) {
